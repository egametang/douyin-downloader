...
class LikeUserModeStrategy(BaseUserModeStrategy):
    ...
    async def collect_items(
        ...
        if not callable(fetcher):
            logger.error("API client missing get_user_like")
            return []

        aweme_list: List[Dict[str, Any]] = []
        max_cursor = 0
        has_more = True
        browser_recovery_needed = False
        ...
        self.downloader._progress_update_step("拉取喜欢列表", "分页抓取中")
        ...
        while has_more:
            ...
            if not page_items:
                if page.get("status_code") == 0:
                    browser_recovery_needed = True
                    logger.warning(
                        "User like API returned no items at cursor=%s, switching to browser fallback",
                        request_cursor,
                    )
                break
            ...
            if has_more and max_cursor == request_cursor:
                logger.warning(
                    "Like max_cursor did not advance (%s), stop paging to avoid loop",
                    max_cursor,
                )
                browser_recovery_needed = True
                break
        ...
        if browser_recovery_needed:
            self.downloader._progress_update_step("拉取喜欢列表", "分页受限，尝试浏览器回补")
            await self.downloader._recover_user_like_with_browser(
